...
async function startServer() {
  ...
  const GATEKEEPER_PASSWORD = "Xiemac123!";
  ...
  async function waitForRestSlot(context: string) {
    ...
    // 2. Maintain 27s Interval
    const now = Date.now();
    const timeSinceLastCall = now - lastRestCallTime;
    
    if (timeSinceLastCall < REST_INTERVAL) {
      const waitTime = REST_INTERVAL - timeSinceLastCall;
      console.log(`[Queue] ${context}: Waiting ${Math.round(waitTime/1000)}s to maintain 27s interval...`);
  ...
  const authMiddleware = (req: express.Request, res: express.Response, next: express.NextFunction) => {
    ...
    if (authCookie === GATEKEEPER_PASSWORD || authToken === GATEKEEPER_PASSWORD) {
      ...
      console.log(`[Auth] Unauthorized access attempt to ${req.path}. Cookie: ${!!authCookie}, Token: ${!!authToken}`);
  ...
  app.post("/api/login", (req, res) => {
    ...
    if (password === GATEKEEPER_PASSWORD) {
      ...
      console.log(`[Auth] Login successful. Secure: ${isSecure}, Proxy: ${req.headers['x-forwarded-proto']}`);
      ...
      addLog(null, 'info', '门卫系统登录成功');
      res.json({ success: true, token: GATEKEEPER_PASSWORD });
    } else {
      addLog(null, 'warn', '门卫系统登录失败：密码错误');
  ...
  function initAccountWS(account: BinanceAccount) {
    ...
    wsClient.on("formattedMessage", (data: any) => {
      ...
      if (state) {
        if (data.eventType === 'ACCOUNT_UPDATE') {
          ...
          // USER REQUEST: Do not update positions or orders from WebSocket push
          // Only broadcast the raw update for potential UI balance updates
          io.emit("binance_update", { accountId: account.id, accountName: account.name, data });
        }
      }
    });

    (wsClient as any).on("open", (data: any) => {
      console.log(`WS Connected for ${account.name}`, data);
      addLog(account.id, 'info', `WebSocket 连接成功 (${account.name})`);
      wsStatuses[account.id] = 'connected';
      io.emit("ws_status", { accountId: account.id, status: 'connected' });
    });
    ...
    (wsClient as any).on("reconnecting", (data: any) => {
      console.log(`WS Reconnecting for ${account.name}: Attempt ${data.attempt}, Delay: ${reconnectDelay}ms`);
      addLog(account.id, 'warn', `WebSocket 正在重连 (${account.name}): 第 ${data.attempt} 次尝试`);
      wsStatuses[account.id] = 'reconnecting';
      io.emit("ws_status", { accountId: account.id, status: 'reconnecting' });
      reconnectDelay = Math.min(reconnectDelay * 1.5, maxReconnectDelay);
    });

    (wsClient as any).on("reconnected", (data: any) => {
      console.log(`WS Reconnected for ${account.name}`);
      addLog(account.id, 'info', `WebSocket 重连成功 (${account.name})`);
      reconnectDelay = 5000;
      wsStatuses[account.id] = 'connected';
      io.emit("ws_status", { accountId: account.id, status: 'connected' });
      // USER REQUEST: Do NOT fetch state on reconnect, wait for WS push
    });

    (wsClient as any).on("error", (err: any) => {
      console.error(`WS Error for ${account.name}:`, err);
      addLog(account.id, 'error', `WebSocket 错误 (${account.name}): ${err.message || JSON.stringify(err)}`);
      wsStatuses[account.id] = 'error';
      io.emit("ws_status", { accountId: account.id, status: 'error' });
    });
  ...
  const startupAccounts = async () => {
    console.log(`[Startup] Beginning staggered initialization for all active accounts...`);
    const dbAccounts = getAccounts();
  ...
  setInterval(() => {
    ...
    if (min !== lastProcessedMin) {
      ...
      if (hour === 8) {
        accounts.forEach((account, index) => {
          ...
          if (min === tradeMin) {
            ...
            console.log(`[Daily Sync] ${now.toLocaleTimeString()}: Syncing trades for ${account.name}...`);
          ...
          if (min === incomeMin) {
            ...
            today8am.setHours(8, 0, 0, 0);
            const yesterday8am = new Date(today8am.getTime() - 24 * 60 * 60 * 1000);
            console.log(`[Daily Sync] ${now.toLocaleTimeString()}: Syncing incomes for ${account.name}...`);
      ...
      if (min === 27 || min === 57) {
        console.log(`[WS Maintenance] ${now.toLocaleTimeString()}: Refreshing all active WebSockets...`);
